...
doc = """
Implicit Association Test shown after the dictator-game block.

The intro app sets treatment=True for participants routed to this post-DG IAT.
"""


class Constants(BaseConstants):
    name_in_url = 'association_1'
...
class Intro(Page):
    ...
    def is_displayed(player):
        return player.round_number == 1 and player.participant.vars.get('treatment', True)
    form_model = 'player'
...
class RoundN(Page):
    @staticmethod
    def is_displayed(player):
        return player.participant.vars.get('treatment', True)

    template_name = "iat_2/Main.html"
...
class Results(Page):
    @staticmethod
    def is_displayed(player):
        return player.round_number == 7 and player.participant.vars.get('treatment', True)
