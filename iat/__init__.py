...
doc = """
Implicit Association Test shown before the dictator-game block.

The intro app sets treatment=False for participants routed to this pre-DG IAT.
"""


class Constants(BaseConstants):
    name_in_url = 'questionnaire_pratiques_pedago'
    ...
    for side in ['left', 'right']:
        for cls in ['primary', 'secondary']:
            if cls in block[side] and params[f"{cls}_images"]:
                # Use the first image in the category as a corner thumbnail.
...
class Trial(ExtraModel):
    ...
    Keeps corner categories from round setup to simplify further analysis.
...
def play_game(player: Player, message: dict):
    ...
    When done solving, client should explicitly request next trial by sending 'next' message
    ...
    now = time.time()
    # Current trial, if the participant refreshed or is retrying.
    ...
    if message_type == "next":
        if current is not None:
            if current.response is None:
                raise RuntimeError("Essayer de passer un essai non résolu")
            if now < current.timestamp + ret_params["trial_delay"]:
                raise RuntimeError("réessai trop rapide")
    ...
    if message_type == "answer":
        ...
        if current.response is not None:  # it's a retry
            ...
            # Undo the previous progress update before recording the retry.
            player.num_trials -= 1
    ...
    if message_type == "cheat" and settings.DEBUG:
        # Generate remaining data for the round during local debugging only.
...
class Intro(Page):
    ...
    def is_displayed(player):
        return player.round_number == 1 and not player.participant.vars.get('treatment', True)
    form_model = 'player'
...
class RoundN(Page):
    @staticmethod
    def is_displayed(player):
        return not player.participant.vars.get('treatment', True)
...
class Results(Page):
    @staticmethod
    def is_displayed(player):
        return player.round_number == 7 and not player.participant.vars.get('treatment', True)
    ...
    def vars_for_template(player: Player):
        ...
        # Category combinations used for the positive D-score direction.
        labels3 = labels_for_block(get_block_for_round(3, player.session.params))
        # Category combinations used for the negative D-score direction.
...
page_sequence = [IAT_intro, RoundN, Results]
